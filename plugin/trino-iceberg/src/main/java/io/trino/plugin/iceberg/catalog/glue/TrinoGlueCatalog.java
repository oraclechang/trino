        ...
        extends AbstractTrinoCatalog
{
            ...
            boolean ignoreExisting)
    {
        ...
        existing.ifPresent(table -> dropMaterializedView(session, table));
    ...
    public void dropMaterializedView(ConnectorSession session, SchemaTableName viewName)
    {
        ...
        dropMaterializedView(session, view);
    }

    private void dropMaterializedView(ConnectorSession session, com.amazonaws.services.glue.model.Table view)
    {
        String schemaName = view.getDatabaseName();
        String viewName = view.getName();
        String storageTableName = view.getParameters().get(STORAGE_TABLE);
        if (storageTableName != null) {
            try {
                dropTable(session, new SchemaTableName(schemaName, storageTableName));
            }
    ...
    public void renameMaterializedView(ConnectorSession session, SchemaTableName source, SchemaTableName target)
    {
        ...
        try {
            ...
            TableInput tableInput = getMaterializedViewTableInput(target.getTableName(), table.get().getViewOriginalText(), table.get().getOwner(), table.get().getParameters());
            CreateTableRequest createTableRequest = new CreateTableRequest()
                    .withDatabaseName(target.getSchemaName())
                    .withTableInput(tableInput);
            stats.getCreateTable().call(() -> glueClient.createTable(createTableRequest));
